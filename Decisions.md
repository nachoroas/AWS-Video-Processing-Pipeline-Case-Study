...
This project uses a queue-based AWS architecture to process long-running videos without blocking the backend or keeping compute resources active at all times.

## Decision Summary

| Decision                              | Reason                                                             |
| ------------------------------------- | ------------------------------------------------------------------ |
| Use S3 as the file boundary           | Keep large video files outside the backend and message queue       |
...
| Use AWS Batch for processing          | Run long-running containerized workloads without permanent workers |
...
S3 stores both inputs and outputs. Other services pass object keys, metadata and job identifiers instead of transferring video payloads. This keeps the architecture simpler and reduces memory, timeout and payload-size risks.
...
SQS also provides a buffer when several videos arrive close together.

## Use Lambda as a Dispatcher

Lambda handles a narrow responsibility: consume messages from SQS, validate job metadata and submit jobs to AWS Batch.

The function does not process video. This keeps execution time short, reduces operational risk and avoids placing heavy dependencies inside a serverless function.

This decision keeps orchestration separate from processing.

## Use AWS Batch for Video Processing

The video-processing workload can run for a long time and requires a controlled execution environment.

AWS Batch fits this workload because it runs containerized jobs and can allocate compute for batch execution. The system avoids permanent workers and starts compute around actual processing demand.
...
The goal is not maximum throughput. The goal is controlled throughput within a defined cost envelope.
...
### Lower operational complexity over custom orchestration

Managed AWS services reduce the amount of infrastructure code required. The tradeoff is tighter coupling to AWS primitives.
...
The system benefits from tracking job states instead of assuming that each event completes. This adds metadata management, but improves observability and recovery.

### S3 object references over direct file transfer

Passing object keys between services avoids large payload transfers. The tradeoff is that each component must handle permissions and object existence checks with care.

## Alternatives Considered

### Always-on EC2 worker

An always-on worker is simpler to reason about, but it keeps compute active even when there are no videos to process. That increases baseline cost.
...
A direct path would reduce the dispatcher layer, but Lambda gives the system a place to validate metadata, enforce constraints and prepare job parameters before execution.
